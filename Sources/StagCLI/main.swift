...
case "version":   print("Stag CLI 1.0.0"); exit(0)
...
func runCapture(_ args: [String]) {
    ...
    let url = URL(string: "stag://capture?type=\(type)")!
...
func runExport(_ args: [String]) {
    guard let filePath = args.first else {
        fputs("Usage: stag export <image-path>\n", stderr)
        exit(1)
    }
    let records = loadHistory()
...
// MARK: - open (open image in Stag editor)

func runOpen(_ args: [String]) {
    guard let filePath = args.first else {
        fputs("Usage: stag open <image-path>\n", stderr)
        exit(1)
    }
    let fileURL = URL(fileURLWithPath: filePath)
    // Try to open with Stag app if running, else fall back to default app
    let stagBundle = "com.ganwar.Stag"
    if let appURL = NSWorkspace.shared.urlForApplication(withBundleIdentifier: stagBundle) {
...
func printHelp() {
    ...
    EXAMPLES:
      stag capture --area --wait
      stag capture --fullscreen --output ~/Desktop/shot.png
      stag history --limit 10 --format json
      stag history --type area --favorites
      stag export ~/Desktop/Stag_shot.png
      stag open ~/Desktop/Stag_shot.png
    """)
}
...
func resolveSavePath() -> URL {
    // Read from Stag's UserDefaults plist
    let prefsPath = NSHomeDirectory() + "/Library/Preferences/com.ganwar.Stag.plist"
    if let dict = NSDictionary(contentsOfFile: prefsPath),
       let data = dict["com.ganwar.Stag.preferences"] as? Data,
       let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
       let path = json["savePath"] as? String {
        return URL(fileURLWithPath: (path as NSString).expandingTildeInPath)
    }
    return URL(fileURLWithPath: NSHomeDirectory() + "/Desktop/Stag Screenshots")
}
...
func loadHistory() -> [[String: Any]] {
    let historyPath = NSHomeDirectory() + "/Library/Application Support/Stag/capture_history.json"
