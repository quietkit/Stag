...
extension Notification.Name {
    ...
    static let stagHotkeysChanged = Notification.Name("stagHotkeysChanged")
}

final class StagAppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let logger = Logger(subsystem: "com.ganwar.Stag", category: "AppDelegate")
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        NotificationCenter.default.addObserver(forName: .stagHotkeysChanged, object: nil, queue: .main) { [weak self] _ in
            self?.installCarbonHotkeys()
        }
        logger.info("Stag launched successfully")
    }
    ...
    private func setupStatusItem() {
        ...
        // ── App ─────────────────────────────────────────────────────────────
        menu.addItem(NSMenuItem.separator())
        menu.addItem(capture("Quit Stag", #selector(quitApp), "q", [.command]))
...
private func capture(_ title: String, _ action: Selector,
        ...
        case "Quit Stag": symbolName = "xmark.circle"
    ...
    private func createEventTap() {
        ...
        let tap = CGEvent.tapCreate(
            ...
            callback: { proxy, type, event, refcon in
                let delegate = Unmanaged<StagAppDelegate>.fromOpaque(refcon!).takeUnretainedValue()
    ...
    private static let accessibilityDismissedKey = "com.ganwar.Stag.accessibilityAlertDismissed"
    ...
    private func showAccessibilityAlert() {
        ...
        DispatchQueue.main.async {
            let alert = NSAlert()
            alert.messageText = "Stag Needs Accessibility Access"
            alert.informativeText = "To use global keyboard shortcuts while you're in other apps, Stag needs Accessibility permission.\n\nSystem Settings → Privacy & Security → Accessibility → add Stag."
