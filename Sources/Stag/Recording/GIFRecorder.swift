...
final class GIFRecorder: NSObject, CaptureRecorder, ObservableObject, @unchecked Sendable {
    ...
    let processingQueue = DispatchQueue(label: "com.ganwar.Stag.gifrecorder.processing", qos: .userInitiated)
    private let encodeQueue = DispatchQueue(label: "com.ganwar.Stag.gifrecorder.encode", qos: .utility)
    ...
    func startCapture(filter: SCContentFilter, config: RecordingConfig, outputURL url: URL) async throws {
        ...
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("stag_gif_\(UUID().uuidString)")
